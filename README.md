...
- **設定** — ルールプリセットの管理。人数（4人/3人）・配給原点・返し点・ウマ・飛び賞を
  ...
  タブバーの表示（絵文字/線画/文字のみ）も選べる（端末ごとの設定。明暗はOSに従う）
...
`DEFAULT_ICONS` は下部タブバーの表示。`emoji`（絵文字）`line`（線画アイコン）
`text`（文字のみ）の3つ。絵文字はOSによって見た目が変わり、選択中でも色が付かない。
線画はSVGなので、どの環境でも同じ形で、選択中はアクセント色になる。

どちらも使う人が設定タブから各自変更できる（端末ごとに記憶される）。存在しない id を
書くと配色は `green`、アイコンは `emoji` にフォールバックするが、`npm test` が
検出して落ちる。
