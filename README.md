...
- **パスワードによるアクセス制御**（任意）。設定すると初回アクセス時に入力を求められる
...
デプロイ前に決めるのは**タイトル**だけでよい。`src/Config.js` の `APP_TITLE` を
書き換えると、ブラウザのタブ・画面上部の見出し・`setup()` が作るスプレッドシートの
ファイル名がまとめて変わる。

```js
var APP_TITLE = '家族麻雀 スコア記録';   // ← ここだけ
```
