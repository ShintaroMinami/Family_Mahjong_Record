...
*表示例（四麻200半荘＋三麻100半荘）を入れた状態。左から 登録・今日・履歴・統計・設定 タブ。*
...
- **画面** — Apps Script の Web アプリとして公開する。`doGet` が単一ページを返し、
  ブラウザからは `google.script.run` でサーバー側の関数を呼ぶ
- **データ** — `setup()` が自分の Google Drive に作るスプレッドシート1つ。中身は普通のデータシートなので、必要なら直接開いて編集したりもできる
- **公開範囲** — 「全員（匿名アクセス可）」で公開し、URLを知っている人はアクセスできる、という運用を想定しています。URL を知られただけでは開かれないよう、一応非常に簡易的なパスワードで守っています。サーバー側の処理はデプロイした本人の権限で動くので、データシート及びその他のドライブ内のデータは他者に共有しません。

### 前提としている技術スタック

**フレームワークもビルド工程もない。** `src/` に置いた素の JavaScript と HTML/CSS が
そのまま動きます。実行時の外部依存はなく、Apps Script のランタイムだけで完結します。

| | |
|---|---|
| 使うのに必要 | Google アカウント（デプロイする人だけ） |
| 開発・デプロイに必要 | Node.js と npm。ローカルサーバー・テスト・`clasp` の実行に使う |
| 型チェック | JSDoc コメント＋TypeScript |
| デプロイ | [clasp](https://github.com/google/clasp)（`npm install` で入る） |
...
まず動かしてみるだけなら、Google へのデプロイなしにブラウザでローカルに確認できる。
...
`npm test` は `src/` の実コードを Node に読み込んで検証。
