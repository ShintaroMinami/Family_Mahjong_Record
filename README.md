...
自分の Google アカウントの中だけで完結する。**サーバーの用意も、月々の費用も要らない。**

- **画面** — Apps Script の Web アプリとして公開する。`doGet` が単一ページを返し、
  ブラウザからは `google.script.run` でサーバー側の関数を呼ぶ
- **データ** — `setup()` が自分の Drive に作るスプレッドシート1つ。DBは立てない。
  中身は普通のシートなので、必要なら直接開いて眺めたり直したりできる
- **公開範囲** — 「全員（匿名アクセス可）」で公開する。使う人に Google アカウントを
  求めないためで、その代わりに URL を知られただけで開かれないようパスワードで守る。
  サーバー側の処理はデプロイした本人の権限で動くので、シート自体は共有しない

### 前提としている技術スタック

**フレームワークもビルド工程もない。** `src/` に置いた素の JavaScript と HTML/CSS が
そのまま動く。実行時の外部依存はゼロで、Apps Script の V8 ランタイムだけで完結する。

| | |
|---|---|
| 使うのに必要 | Google アカウント（デプロイする人だけ） |
| 開発・デプロイに必要 | Node.js と npm。ローカルサーバー・テスト・`clasp` の実行に使う |
| 型チェック | JSDoc コメント＋TypeScript（`.ts` ファイルは書かない） |
| デプロイ | [clasp](https://github.com/google/clasp)（`npm install` で入る） |

Apps Script には**モジュールがなく、全ファイルが1つのグローバルスコープを共有する**。
`src/` に `import` / `export` が出てこないのはそのためで、ローカル実行もこの環境を
再現している（[DESIGN.md](DESIGN.md) §9）。
