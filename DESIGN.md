...
| 統計 | 期間・プレイヤーで絞った通算成績（対局数、平均順位、順位分布、総pt、平均素点、連対率、トップ率、ラス率、トビ率）。四麻と三麻は分けて集計する（ウマ・オカが異なり、順位の取りうる幅も違うため、混ぜた平均は意味を持たない）。見出しをタップすると任意の列で並べ替えられ、成績の推移グラフは合計・平均・平順を切り替えられる。いずれもクライアント側で保持した集計結果を組み替えるだけで、再取得はしない |
