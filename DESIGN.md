...
| 公開範囲 | 匿名アクセス可（URLを知る全員）＋ 合言葉 | 家族にGoogleアカウントを強制しない。合言葉でURL流出時の保険をかける（§10参照） |
...
| 設定 | ルールプリセットの作成・編集・無効化。人数・配給原点・返し点・ウマ・飛び賞を個別に入力し、オカとウマ合計は自動計算して表示・検証する。合言葉の設定・変更・解除もここ |
| 合言葉 | 合言葉が設定されている場合に最初に出る入力画面。認証後は端末に記憶され、次回以降は表示されない |
...
| `apiAuthStatus()` | 合言葉が必要かどうか（**無認証**） |
| `apiVerifyPasscode(candidate)` | 合言葉の照合（**無認証**） |
| `apiSetPasscode(new, current)` | 合言葉の設定・変更・解除 |
...
**匿名公開のまま共有の合言葉で守る**方式を採用した。

### 実装

- 合言葉はスクリプトプロパティ `PASSCODE` に保存する
- **未設定なら認証は行わない**。セットアップ直後は誰でも使える状態で、
  設定タブから合言葉を設定した時点で保護が有効になる
- ガード対象の各サーバー関数は**最終引数**に合言葉を受け取り、先頭で
  `requirePasscode_()` を呼ぶ。フロントの `call()` が自動的に付与する
- 認証が必要なことを示すエラーは `AUTH_REQUIRED` で始まる。フロントはこれを
  検出して合言葉画面に戻す
- `apiAuthStatus` と `apiVerifyPasscode` だけは無認証。合言葉画面自体が
  これらを使うため
...
- 合言葉は端末の `localStorage` に平文で保存される。端末を他人に触られれば読める
- 家族の誰かが合言葉を第三者に教えれば、その人はアクセスできる
- 総当たり攻撃に対するレート制限は設けていない（合言葉を十分な長さにすることで対応）

合言葉を変更すると、他の端末は保存済みの値が古くなるため再入力が必要になる。
